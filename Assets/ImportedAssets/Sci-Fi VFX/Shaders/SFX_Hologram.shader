...
Shader "QFX/SFX/Hologram"
{
	Properties
	{
		...
		_TimeRandomisation("TimeRandomisation", Float) = 0
		...
		_GlitchInterval("Glitch Interval", Range( 0 , 10)) = 2
	...
	SubShader
	{
		...
		void vertexDataFunc( inout appdata_full v, out Input o )
		{
			...
			float temp_output_100_0 = ( step( _Glitch , sin( ( (_GlitchSpeed * (_Time.y + _TimeRandomisation) ) + ase_vertex3Pos.y ) ) ) * step( 0.5 , sin( ( (_Time.y + _TimeRandomisation) * _GlitchInterval) ) ) );
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float2 Scan35 = step( frac( ( ( _ScanTiling * ase_worldPos.y ) + ( (_Time.y + _TimeRandomisation) * _ScanSpeed ) ) ) , temp_cast_2 );
			float2 Glow57 = frac( ( ( _GlowTiling * ase_worldPos.y ) + ( (_Time.y + _TimeRandomisation) * _GlowSpeed ) ) );
			float4 Flicker72 = tex2D( _FlickerTexture, ( (_Time.y + _TimeRandomisation) * _FlickerSpeed ) );
			float4 Opacity149 = ( MainAlpha36 * ( float4( Scan35, 0.0 , 0.0 ) + Rim8 + float4( Glow57, 0.0 , 0.0 ) ) * Flicker72 );
			o.Alpha = Opacity149.r * _AlphaValue;
		}
